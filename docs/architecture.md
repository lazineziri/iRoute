...
The API has two explicit identity profiles. `DevelopmentHeaders` accepts local tenant/actor headers for a credential-free developer loop. `Jwt` validates bearer tokens against a configured authority and audience, requires a tenant claim, derives actor identity from a claim, and ignores caller-controlled scope headers. Runtime stores and reuse indexes remain tenant-scoped as a second line of isolation.
