...
```bash
dotnet add MyApp.csproj reference ../iRoute/src/Clients/iRoute.Sdk.DotNet/iRoute.Sdk.DotNet.csproj
```

Or build and consume a local package:

```bash
dotnet pack ../iRoute/src/Clients/iRoute.Sdk.DotNet \
...
```bash
dotnet build src/Clients/iRoute.Sdk.DotNet/iRoute.Sdk.DotNet.csproj --configuration Release
```
